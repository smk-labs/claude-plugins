...
/* That side is read off #card, never off a logical inset. #rcmenu forces direction:ltr on ITSELF (its dots and labels must not mirror), and inset-inline-* resolves against the element's own writing mode, so inset-inline-end was always physical right. 4.14.0 claimed both directions on that basis and only ever worked in LTR: every Persian card wore the dots on top of its title (field bug, fixed 4.17.0). Physical right plus a #card[dir=rtl] flip says what it actually does. */
