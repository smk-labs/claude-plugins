...
const BRIDGE_JS = [
  ...
  "window.sendPrompt=function(t){rpc('ui/message',{role:'user',content:{type:'text',text:String(t)}},function(res,err){if(err&&window.__rcToast)window.__rcToast('action failed: host rejected ui/message')})};",
  ...
  "if(m.id!=null&&!m.method&&pending[m.id]){var cb=pending[m.id];delete pending[m.id];cb(m.result,m.error);return}",
