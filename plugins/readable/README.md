...
- Protocol-tested: `node plugins/readable/mcp/test.js` runs a 95-check JSON-RPC exchange, including capability negotiation (`io.modelcontextprotocol/ui`), the `roots/list` save-dir handshake, template mime `text/html;profile=mcp-app`, `_meta.ui.resourceUri` linkage, and the no-UI-host fallback (tool answer tells the model to fall back and re-deliver as text/widget).
