...
<script src="https://cdn.jsdelivr.net/gh/smk-labs/claude-plugins@readable-v2.1.0/plugins/readable/assets/rtl-card.js"></script>

A local renderer turns it into a polished RTL card; you write zero HTML and zero styling. Put code, paths, and URLs in backticks; never write a literal closing script tag inside the markdown. Keep any text outside the card short and in English. One-or-two-sentence replies: skip the widget, answer as BiDi-safe plain text (start lines with a strong RTL character, no trailing Latin token). Build an SVG/diagram (readable:visualize skill) only when the user explicitly asks to see something visual.
</readable-rtl-rule>
