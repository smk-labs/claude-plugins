...
- **GitLab group**: which group/namespace the project lives under. On self-hosted GitLab, runners are often attached at the group level. If the project sits in the wrong group, stop and tell the user to move it first: CI will hang otherwise.
- **Runner tag(s)**: the GitLab runner tag that can reach the target cluster. The user gets this from their platform team or GitLab admin. If they have separate tags for heavy jobs (kaniko) vs light jobs (helm/kubectl), capture both. On gitlab.com shared runners, no tag is needed; drop the `tags:` blocks.
- **Namespace**: the Kubernetes namespace the user can deploy to (e.g. `team-alpha-dev`).
- **Environment label**: `staging` / `prod` / `dev`.
- **Public host** (optional): FQDN for the routing object, e.g. `myapp.example.com`. If none, skip the routing template entirely.
- **Routing layer**: confirm from Step 1:
  - **Gateway API**: ask for the parent Gateway name + namespace (commonly something like `external-gateway` in `gateway-system` or a platform-owned namespace) and the listener section name.
  - **ingress-nginx**: ask for the TLS secret name if HTTPS terminates at the ingress.
...
Confirm the user has already added the kubeconfig as `KUBE_CONFIG_B64` (Masked + Hidden CI/CD variable, Settings > CI/CD > Variables). Hidden prevents it from being echoed in job logs; Masked keeps it redacted if it ever leaks into an echo. If not set yet, give them the encode commands:

```bash
# macOS
base64 -i ~/Downloads/my.kubeconfig | pbcopy
# Linux (no line wrapping: masked vars must be single-line)
base64 -w0 ~/Downloads/my.kubeconfig
```
...
| `build` | **always**: kaniko to GitLab Container Registry |
...
Cover this explicitly in the generated charts based on what Step 2 told you. Translate "routing object" below into the right kind for the cluster (`Ingress` for nginx clusters, `HTTPRoute` for Gateway API clusters):

- **Same-host `/api`**: the public-facing chart (frontend) gets two routing rules: `/api` to the backend service, `/` to the frontend service. The backend chart's routing template stays disabled. On Gateway API this is two `rules[]` entries on one `HTTPRoute` with `matches: [{ path: { type: PathPrefix, value: /api } }]` and `matches: [{ path: { type: PathPrefix, value: / } }]` respectively.
...
- **Build-time baked URL** (Vite `VITE_API_URL`, CRA `REACT_APP_API_URL`): pass via a build arg in the **frontend's** `build_image` job. Don't put it in `values.yaml` `env:`. It's baked into the bundle, not read at runtime.
...
The parent `Gateway` is **shared cluster infrastructure**. Don't create one from this chart. Ask the platform team for its name + namespace (e.g. `external-gateway` in `gateway-system`). `sectionName` usually points at the HTTPS listener, which is what you almost always want. TLS termination happens at the `Gateway`, not the `HTTPRoute`, so you don't ship a `tls` block in the chart for Gateway API clusters.
...
- `values.yaml` defaults:
  ...
  - `service` (ClusterIP), routing block (disabled by default), `autoscaling` (disabled)
...
- `values-<env>.yaml`: per-env overrides (host, TLS secret, resource limits).
- `templates/_helpers.tpl`: `name`, `fullname`, `chart`, `labels`, `selectorLabels`, `serviceAccountName`, `secretName`.
- `templates/deployment.yaml`: one container, `envFrom` pointing at a ConfigMap (non-secret) + Secret (app secrets), probes, resources, **`imagePullSecrets` from `.Values.imagePullSecrets`**.
- `templates/service.yaml`.
- **Routing template, pick exactly one based on the cluster** (see Step 1's routing-layer rule):
  - `templates/ingress.yaml` for **ingress-nginx** clusters: gated on `.Values.ingress.enabled`, sets `ingressClassName: nginx`. For SSE / long-poll apps include `nginx.ingress.kubernetes.io/proxy-buffering: "off"` and 300s read/send timeouts.
  - `templates/httproute.yaml` for **Gateway API** clusters: gated on `.Values.httpRoute.enabled`, references a parent `Gateway` via `parentRefs:` (name + namespace come from `.Values.httpRoute.parentRef`). Use `gateway.networking.k8s.io/v1`. Do **not** also generate an `Ingress`. Pick one.
...
> **Every job must have a `tags:` block** matching the runner tag from Step 2 (skip this only on gitlab.com shared runners). Without the right tag, jobs sit `pending` indefinitely. Convention: heavy jobs (kaniko build) on the heavy-pool tag if one exists, light jobs (helm/kubectl/lint/test) on the cluster-reachable tag. If only one tag exists, use it for everything.

On Kubernetes-executor runners you can request more resources per job (the runner enforces ceilings):
...
|---|---|---|---|
| `KUBE_CONFIG_B64` | base64-encoded kubeconfig | yes + Hidden | optional |
| `GITLAB_DEPLOY_TOKEN_USER` | username of the deploy token (Step 7) | yes | optional |
| `GITLAB_DEPLOY_TOKEN` | password of the deploy token (Step 7) | yes | optional |
| one row per app secret | injected via `--set-string secrets.data.<KEY>=$<KEY>` | yes | optional |
...
### Build job (kaniko to GitLab Container Registry)

```yaml
build_image:
  stage: build
  tags:
    - <heavy-runner-tag>   # fall back to the deploy tag if no heavy pool
  image:
...
deploy:
  stage: deploy
  tags:
    - <deploy-runner-tag>   # must reach the target cluster
...
If the kubeconfig holds one context per namespace, either use `kubectl -n <namespace>` on each command (as above) or pin a context with `kubectl config use-context <cluster>-<namespace>`.
...
- **Per-service image name**: push to `$CI_REGISTRY_IMAGE/backend:$IMAGE_TAG` and `$CI_REGISTRY_IMAGE/frontend:$IMAGE_TAG` so the two images are distinct in the registry. Set kaniko `--destination` accordingly.
...
The image lives in a **private** GitLab project registry. Most Kubernetes clusters have no native integration for GitLab's registry, so the standard path is a Kubernetes `docker-registry` secret referenced via `imagePullSecrets`.
...
- **Name**: `k8s-pull`
- **Username**: leave blank (GitLab generates one like `gitlab+deploy-token-12345`) or set e.g. `gitlab+deploy-token-k8s`
- **Scopes**: `read_registry` only, nothing else

They save the generated **username** and **token** as GitLab CI/CD variables `GITLAB_DEPLOY_TOKEN_USER` and `GITLAB_DEPLOY_TOKEN` (Masked). The token is not viewable again after the page closes.
...
If the user deploys many apps from the same GitLab group to the same namespace, they can create the secret **once manually** and drop the `kubectl create secret` step from CI. Use the registry hostname of their GitLab instance as `--docker-server` (`registry.gitlab.com` for gitlab.com, or the self-hosted registry host):
...
Then set `imagePullSecrets: [{ name: <APPNAME>-reg-secret }]` in `values.yaml` (instead of `gitlab-registry`). If the token ever rotates, re-run the command with `--dry-run=client -o yaml | kubectl apply -f -` appended so the existing secret is replaced in place.
...
3. **Assumptions to verify**: runner tag correct?, health endpoint, exposed port matches entrypoint, migrations present?, host/TLS, resource sizing.
4. **First-deploy checklist**: push to default branch, watch the pipeline, `kubectl -n <ns> get pods,svc,ingress` (or `httproute`), hit the URL.

### Common first-deploy issues

| Symptom | Likely cause |
|---|---|
| Pipeline sits pending forever | No runner matches the job's `tags:` (wrong tag, or project in a group without runners). Fix the tag or move the project. |
| `ImagePullBackOff` | Pull secret missing or wrong. Re-create the `docker-registry` secret in the namespace and check `imagePullSecrets` in the chart. |
| `kubectl` says forbidden | The kubeconfig's permissions are namespace-scoped. Always pass `-n <your-namespace>` and confirm the namespace is one the credentials cover. |
| Routing object healthy but the hostname 404s or doesn't resolve | The DNS record for the host doesn't exist yet, or points at the wrong load balancer. Create/fix DNS first. |
| App is up but health checks fail | The container's `EXPOSE`d port doesn't match what the app binds to, or the probe path is wrong. Check `livenessProbe` / `readinessProbe` in Helm values. |

## House rules

- **Project must be under the correct GitLab group** when runners are attached at group level. Confirm with the platform team or GitLab admin before generating CI.
- **Every job needs a `tags:` block** with the team's runner tag (except gitlab.com shared runners). No tag = stuck pipelines. Get the tag from the platform team; do not guess.
...
- **Routing layer is cluster-determined, not optional.** Gateway API clusters get an `HTTPRoute` (no `Ingress`); ingress-nginx clusters get an `Ingress` (no `HTTPRoute`). Never ship both. If the cluster's routing layer is unknown, ask before generating.
...
- **Long-poll / SSE apps on ingress-nginx** need `proxy-buffering: "off"` + extended (300s) ingress timeouts. On Gateway API, configure equivalent timeouts via `BackendTrafficPolicy` / `HTTPRoute` `timeouts` instead; ask the platform team if the cluster has a standard policy.
