...
3. **Model: two quota pools.** `--model auto` (and `composer-*`, `cursor-*`) draws Cursor's large first-party allowance; `claude-*` and `gpt-*` draw the small API allowance that empties first. Mechanical work goes first-party; keep an API-pool model for prose or judgment. Say which you used. Full rule: the **cursor-orchestrate** skill, "Model routing".
