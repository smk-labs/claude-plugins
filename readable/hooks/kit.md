...
@import url('https://cdn.jsdelivr.net/gh/smk-labs/claude-plugins@readable-v6.4.1/readable/assets/rc.css');
...
- Trend sparkline (time series: x evenly spaced 0..100 oldest→newest, y inverted so 2≈max and 28≈min; optional area fill <polygon points="0,30 …same points… 100,30"/>; optional second series <polyline class="s2">): <div class="spark"><svg viewBox="0 0 100 30" preserveAspectRatio="none"><polyline points="0,26 25,19 50,22 75,10 100,4"/></svg><div class="x"><span>فروردین</span><span>تیر</span></div></div>.
...
- Hub (one thing connected to many, up to eight legs; `out` on a leg turns its arrow around, which is the difference between "reads from" and "writes to"): <div class="hub"><div class="c">centre<span>note</span></div><div class="s">item<span>note</span></div><div class="s out">item</div>...</div>. Nine items or more is a tree: <div class="hub tree"> is a root, branches and leaves with no arrows, for items that group, and a branch may nest its own <div class="s"> children.
...
- Link to another document, drawn as a document rather than a kv row: <a class="preview" href="…"><b>title</b><span>one line of context</span><small>host</small></a>. Use it when a link IS the point (a spec, a prototype, a related report); a link inside a sentence stays a plain <a>. Write the host as the bare domain, no scheme and no path.
- Section breaks for a long document: add `sections` to the wrapper — <div class="numbered sections"> — and every direct-child h3 gets a hairline rule above it, room to breathe and one size up. Nothing per heading. Use it when the sections are long enough that a reader scrolling fast would otherwise miss where one ends.
